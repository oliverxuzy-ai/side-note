...
v1 支持：标题（H1-H3）/ 段落 / 无序列表（真 • 圆点）/ 有序列表 /
**任务列表 `- [ ] / - [x]`（真复选框，可点击切换）** / 行内 code / 代码块 /
引用块 / 粗体 & 斜体 / 链接。

v1.1 推迟：图片、表格、脚注、删除线、嵌套引用、HTML 块。
...
| 2026-05-16  | 有序列表纳入 live-edit；列表统一悬挂缩进；代码块/引用块补齐 DESIGN 视觉 | 用户实测："number bullet 没做效果"。审计发现有序列表无 regex/渲染路径，且代码块无底/无边、引用无立柱、行内 code 无底——均 DESIGN v1 已规定但未落地。调研 Bear/Obsidian/CodeMirror：proportional 字体下悬挂缩进是 live-edit「做得好」第一杠杆。`MarkdownLayoutManager.drawBackground` 画圆角代码块底+hairline / 引用 2pt sage 立柱；`NSParagraphStyle.headIndent` 让换行对齐内容。数字标记保留可见（语义非语法），只挂缩进 |
| 2026-05-16  | 呼出快捷键从硬编码 ⌃⇧Space → Preferences 可录制自定义 | 用户要求。`RevealHotkey` 用 HotKey 库 `KeyCombo.dictionary` 存 UserDefaults（carbon 码跨布局稳定），post `didChange` → `HotkeyService` 热重绑无需重启。录制控件纯 AppKit NSView 收 keyDown（零新依赖），要求 ≥1 个 ⌘/⌃/⌥，Esc 取消 ⌫ 复位 |
| 2026-05-16  | 新增 slash 命令菜单（/ 唤起插入 标题/列表/待办/有序/引用/代码块） | 用户要求。轻量浮层 NSPanel（nonactivating，不抢焦点）+ SwiftUI 列表；方向键/回车/Tab/Esc 由 `NSTextViewDelegate.doCommandBy` 拦截驱动，文本视图始终第一响应者。视觉沿用 DESIGN：半透白卡 + hairline + sage 选中软底+左立柱。零新依赖 |
| 2026-05-16  | 版本号自动化：VERSION 单一真相 + sync 脚本 + release 脚本 + GitHub Action | 用户要求。`VERSION`→`scripts/sync-version.sh` 写入 project.yml（MARKETING_VERSION=VERSION，CURRENT_PROJECT_VERSION=git commit 数，单调递增）；tag push `v*` 触发 workflow 复用既有 dmg 流水线，**仅建 draft release**（遵守 HANDOFF「不擅自公开发布」）。不改用户本地未跟踪的 pre-push hook（additive/可逆） |
| 2026-05-16  | GitHub 仓库显示 "Other" 而非 "MIT License" 修正 | LICENSE 文件附加了第三方字体说明段，licensee 文本相似度跌破阈值 → 回退 Other。LICENSE 改回纯标准 MIT；字体声明移到 `THIRD-PARTY-NOTICES.md`，README 引用同步 |
