# Implementation Plan — HoverNote v1
...
**DoD**：在 Xcode 里 `⌘R` 能跑出一个空白 macOS 窗口，window title 是 "HoverNote"。
...
- **首启动 macOS 会弹一次「允许 HoverNote 访问 Documents 文件夹」系统框**——`~/Documents/SideNote` 受 TCC 保护，点一次 Allow 即可（真实用户一次性行为；自动化测试用临时目录绕开验证逻辑）
...
可删/改），附件 `HoverNote-0.1.0.dmg`（1.9M，adhoc 签名，本地全链路验证通过）。
...
| `.pkg`    | 安装器（适合复杂安装：多文件、需要后台脚本等） | 不需要。HoverNote 是单 .app，没安装步骤                                  |
